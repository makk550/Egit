...
setCategory.add(t.category__c); 
                ...
                if(Trigger.isinsert  && Trigger.isBefore &&  t.Subject == 'Restoration'){ 
                    t.RecordTypeId = RestorationTaskRecType;
    ...
    if(caseSetSupportRecordType!=null && caseSetSupportRecordType.size()>0){
        for(Case cse : [select id, BusinessHoursId from Case where id in :caseSetSupportRecordType and Severity__c='4'])
    ...
    if(caseIdsToUpdSev!=null && caseIdsToUpdSev.size()>0){
        ...
        Map<id,Case> caseMap = new Map<id,Case>([Select id, OwnerId,tpc_team__c, Severity__c,Origin,BusinessHoursId,lastmodifieddate,Reopen_Count__c,Offering_Name__c,createdDate,AccountId from Case Where id in : caseIdsToUpdSev and Status!='Closed']);
    ...
    if(OpenCallBackCaseIds!=null && OpenCallBackCaseIds.size()>0){        
        List<Case> caseReccordList = [Select Id,Open_CB__c,Next_Action_Due_Date__c from case where Id in:OpenCallBackCaseIds and Open_CB__c!='Y'];
    ...
    if(!UpdInternlPriorityOfCaseSet.isEmpty())
        updateCaseSet.addAll(UpdInternlPriorityOfCaseSet);
    
    if(!updCaseCallBackSet.isEmpty())
        updateCaseSet.addAll(updCaseCallBackSet);
    
    if(!updatePriorityCaseSet.isEmpty())
        updateCaseSet.addAll(updatePriorityCaseSet);        
...
*/
    ...
    if(Trigger.isDelete){
        for(Task taskRec:Trigger.old){
            if((taskRec.RecordTypeId==Label.Other_Support_Task_Record_Type || taskRec.RecordTypeId ==Label.Service_cloud_Task_Record_Type) && !((currentProfileName!=null && ( currentProfileName.contains( 'Admin')|| (currentProfileName.contains('Integration')) )) )){
    ...
    if(Trigger.isUpdate && Trigger.isBefore){
        for(Task taskRec:Trigger.New){                    
            ...
            if((taskRec.RecordTypeId==Label.Other_Support_Task_Record_Type || taskRec.RecordTypeId ==Label.Service_cloud_Task_Record_Type) && (taskRec.Status=='Closed' && oldTaskRec.Status==taskRec.status)&& !((currentProfileName!=null && ( currentProfileName.contains( 'Admin')|| (currentProfileName.contains('Integration')) )) )){
    ...
    if(Trigger.isUpdate && Trigger.isBefore){
        for(Task taskRec:Trigger.New){                    
            task oldTaskRec = trigger.oldmap.get(taskRec.id);
            if((taskRec.RecordTypeId==Label.Other_Support_Task_Record_Type || taskRec.RecordTypeId ==Label.Service_cloud_Task_Record_Type) && ((taskRec.Status=='Open' || taskRec.Status=='In Progress' || taskRec.Status=='Not Started') && (oldTaskRec.Status=='Closed' || oldTaskRec.Status=='Completed'))&& !((currentProfileName!=null && ( currentProfileName.contains( 'Admin')|| (currentProfileName.contains('Integration')) )) )){
    ...
    if((Trigger.isInsert || Trigger.isUpdate)&& (processCalculatedStatusTaskSet!=null && processCalculatedStatusTaskSet.size()>0)){
        List<Case> caseUpdateList = [Select Id , Status , Next_Action__c,Calculated_Status_Transfer_In_Count__c,Calculated_Status_Transfer_Out_Count__c,Date_Time_ReOpened__c,Createddate,Is_Task_Closed__c,Troubleshooting_Stage__c  from Case where Id IN :caseIdSet ];
        ...
        if(caseUpdateList!=null && caseUpdateList.Size()>0){     
            ...
            gssLst = [select id,Case__c,Current_Next_Action__c,Current_Status__c,Current_Status_Added_By__c,Curr_Next_Action_By__c,Current_Next_Action_Date__c,
                      ...
                      Previous_GSS_TR__c,Prev_Next_Action_Date__c,Prev_Next_Action_By__c,Prev_Next_Action_Due_Date__c,Previous_GSS_TR_Date__c,Status_From__c, Case_Owner__c,Case_Severity__c
            ...
            for(Task taskRec : processCalculatedStatusTaskSet){
                if(caseIdMap.containsKey(taskRec.whatId)){
                   ...
                   if(taskRec.Status !='Closed'){
                        ...
                        caseRec.Calculated_Status_Transfer_Out_Count__c = caseRec.Calculated_Status_Transfer_Out_Count__c+1;
                    }
    ...
    if((Trigger.isAfter && (trigger.isupdate || trigger.isinsert))||(trigger.isbefore && trigger.isDelete))
    {
        ...
        if(CaidSet!=null)
            caseidList=[select id from case where id in :CaidSet and status!='Closed'];
        if(caseidList!=null && caseidList.size()>0)
        {
            ...
            catch(Exception ex)
            {
                ...
                if(!Trigger.isDelete)
                    Trigger.new[0].addError(msg+' on the Case.');
                else
                    Trigger.old[0].addError(msg+' on the Case.');
            }
        }
        //US78177 - Commented as part of US78177
        /*

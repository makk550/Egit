...
trigger Renewals_RollupRPToRenewal on Renewal_Product__c (after insert, after update, after delete){
    ...
    if(Trigger.isInsert){    
        for(Renewal_Product__c rp : trigger.new){        
            ...
            || rp.Revenue_Per_Day__c != 0 || rp.Current_Ann_Existing_Maintenance_LC__c != 0)
             && rp.Renewal__c != null){        
    ...
    if(renewalsIds.size() > 0){   
        Map<Id,Renewal__c> renewalsToUpdate = new Map<Id,Renewal__c>([SELECT Id, Name,
                                    ...
                                    Revenue_Per_Day__c , Current_Ann_Existing_Maintenance_LC__c,
                                    (SELECT Id, OCV__c, Annual_OCV_LC__c,Projected_Renewal_LC__c, Raw_Maintenance_Calc__c, ATTRF_CRV__c,
                                    Current_Ann_Existing_Maintenance_LC__c,Revenue_Per_Day__c FROM Renewal_Products__r)

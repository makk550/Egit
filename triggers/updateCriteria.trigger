    ...
    if(trigger.isupdate)
    {
        for(TAQ_Organization__c tOrg: trigger.new)
        {
            if(tOrg.Criteria__c!=null)
            {
                ...
                if((tOrg.Plan_Type__c == 'FY16-Account Development' || tOrg.Plan_Type__c == 'FY16-RTM Partner Account Dev' || tOrg.Plan_Type__c == 'FY16-Account Development 3') && tOrg.Criteria__c != null)
                {
            ...
            else if(trigger.oldmap.get(tOrg.id).Plan_Type__c != tOrg.Plan_Type__c && 
            (trigger.oldmap.get(tOrg.id).Plan_Type__c == 'FY16-Account Development' || trigger.oldmap.get(tOrg.id).Plan_Type__c == 'FY16-RTM Partner Account Dev' || trigger.oldmap.get(tOrg.id).Plan_Type__c == 'FY16-Account Development 3') &&
            (tOrg.Plan_Type__c != 'FY16-Account Development' || tOrg.Plan_Type__c != 'FY16-RTM Partner Account Dev' || tOrg.Plan_Type__c != 'FY16-Account Development 3'))
            {
                tOrg.Criteria_1__c = '';
                tOrg.Criteria_2__c = '';
                tOrg.Criteria_3__c = '';

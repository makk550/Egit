trigger LeadView_AccountOwners on Lead (before insert, before update) 
{
  ...
  */
  ...
  {
  Map<Id,Account> newAccts = new Map<id,Account>([select id,GEO__c,Sales_Area__c,Sales_Region__c,Region_Country__c,Country_Picklist__c from Account where id in :leadaccids]);

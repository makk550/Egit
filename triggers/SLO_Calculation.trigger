trigger SLO_Calculation on Task (before insert) {
  //log.startTrigger('SLO_Calculation');
     ...
     for(Task t: Trigger.New){
        ...
        if( t.RecordTypeId == SupportRecType && t.WhatId!=null && caseMap.get(t.WhatId)!=null){
           ...
           if(caseMap.get(t.WhatId).Severity__c=='1' && SLO_Milestones__c.getValues('Severity1')!=null){
            ...
            t.SLO2_Milestone2__c=BusinessHours.addGmt(caseMap.get(t.WhatId).BusinessHoursId,targTime,(long.Valueof(SLO_Milestones__c.getValues('Severity2').SLO_Milestone_2__c)*long.Valueof(conMilli)));//old value:4800000
            t.SLO3_Milestone3__c=BusinessHours.addGmt(caseMap.get(t.WhatId).BusinessHoursId,targTime,(long.Valueof(SLO_Milestones__c.getValues('Severity2').SLO_Milestone_3__c)*long.Valueof(conMilli)));
            t.SLO4_Milestone4__c=BusinessHours.addGmt(caseMap.get(t.WhatId).BusinessHoursId,targTime,(long.Valueof(SLO_Milestones__c.getValues('Severity2').SLO_Milestone_4__c)*long.Valueof(conMilli)));//old Value:6900000
        }  

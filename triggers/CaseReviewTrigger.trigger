trigger CaseReviewTrigger on Case_Review__c (before insert,after insert,before delete) {
    ...
    if(trigger.isBefore && trigger.isInsert){
        ...
        List<Case> caseList = [select caseNumber,(select name,Case__c,Reviewer_Name__c,Engineer_Name__c from Case_Reviews__r)
        ...
        if(!caseList.isEmpty()){
            for(Case c: caseList){
                ...
                for(Case_Review__c cr: c.Case_Reviews__r){
                    String s = String.valueOf(cr.Reviewer_Name__c)+String.ValueOf(cr.Engineer_Name__c)+String.valueOf(cr.Case__c);
    ...
    if(Trigger.isAfter && Trigger.isInsert){
        ...
        for(Case_Review__c cr: Trigger.new){
            ...
            if(cr.Engineer_Name__c!=cr.Reviewer_Name__c){

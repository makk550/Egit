   ...
   static boolean testFlag = true;
    ...
    if(Trigger.isAfter && Trigger.isInsert){
       ...
       Map<String,Quote> quoteMap = new Map<String,Quote>();
        ...
        for( Product2 product : [select p.id, p.SKU__c,p.Disti_Discount__c, p.description,p.Product_Group__c from Product2 p where p.SKU__c  IN :SkuIds] ){
       ...
       SystemIdUtility.skipUpdateCoTermTrigger = true;
        ...
        for(QuoteLineItem ql:[select qt.id,qt.QuoteId,qt.MSRP_total__c,qt.MSRP_Unit_Cost__c, 
                ...
                skuset.add(ql.sku__c);
                 ...
                 for( Product2 product : [select p.id, p.SKU__c,p.Disti_Discount__c, p.description,p.Product_Group__c from Product2 p where p.SKU__c  IN :skuset] ){
            mProduct.put(product.SKU__c, product);
            }
     ...
     System.debug('entered in Before Insert block');
           ...
           for(Product2 prod: [ select id, SKU__c from Product2 where SKU__c IN : skus])
           {
